const tf = require('@tensorflow/tfjs-node');
const InputError = require('../exceptions/InputError');

async function predictClassificationEmotion(model, image) {
    try {
        // Decode JPEG image, resize, expand dimensions, and convert to float tensor
        const tensor = tf.node
            .decodeJpeg(image)
            .resizeNearestNeighbor([224, 224])
            .expandDims()
            .toFloat();
        
        // Normalize pixel values to [0, 1] range (assuming image data is in [0, 255])
        const normalizedTensor = tensor.div(tf.scalar(255));
        
        // Predict with the model
        const prediction = model.predict(normalizedTensor);
        
        // Extract probabilities from prediction
        const probabilities = await prediction.data();

        // Define class labels
        const labels = ['Natural', 'anger', 'fear', 'joy', 'sadness', 'surprise'];

        // Determine predicted class index
        const predictedClassIndex = tf.argMax(prediction, 1).dataSync()[0];

        // Get the predicted label
        const label = labels[predictedClassIndex];

        // Log probabilities, predicted class index, and label
        console.log(`Probabilities: ${probabilities.join(', ')}`);
        console.log(`Predicted class index: ${predictedClassIndex}`);
        console.log(`Predicted label: ${label}`);

        return { label };
    } catch (error) {
        throw new InputError(`Prediction error: ${error.message}`);
    }
}

module.exports = predictClassificationEmotion;
